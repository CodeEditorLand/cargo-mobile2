...
pub enum Error {
	...
	DirectoryCreationFailed { path: PathBuf, cause: std::io::Error },
	DirectoryReadFailed { path: PathBuf, cause: std::io::Error },
	DirectoryRemoveFailed { path: PathBuf, cause: std::io::Error },
	AssetDirSymlinkFailed(ln::Error),
	DotCargoGenFailed(ndk::MissingToolError),
	FileCopyFailed { src: PathBuf, dest: PathBuf, cause: std::io::Error },
	...
	fn report(&self) -> Report {
		match self {
			...
			Self::DirectoryRemoveFailed { path, cause } => {
				Report::error(format!("Failed to remove directory directory at {:?}", path), cause)
			}
			...
			Self::FileCopyFailed { src, dest, cause } => {
				Report::error(format!("Failed to copy file at {:?} to {:?}", src, dest), cause)
			}
...
pub fn gen(
	...
	bike.filter_and_process(
		...
		|map| {
			...
			map.insert(
				"abi-list",
				Target::all().values().map(|target| target.abi).collect::<Vec<_>>(),
			);
			...
			map.insert("android-app-dependencies", metadata.app_dependencies().unwrap_or_default());
			map.insert(
			...
			map.insert(
				...
				metadata.vulkan_validation().unwrap_or(DEFAULT_VULKAN_VALIDATION),
			);
			...
			let asset_packs = asset_packs.iter().map(|p| p.name.as_str()).collect::<Vec<_>>();
	...
	let asset_pack_src =
		Pack::lookup_platform(ASSET_PACK_TEMPLATE_PACK).map_err(Error::MissingPack)?.expect_local();
	...
	for source in metadata.app_sources() {
		...
		let source_file =
			source_src.file_name().ok_or_else(|| Error::AssetSourceInvalid(source_src.clone()))?;
		fs::copy(&source_src, source_dest.join(source_file)).map_err(|cause| {
			Error::FileCopyFailed { src: source_src, dest: source_dest.clone(), cause }
		})?;
	}
...

	{
		for target in Target::all().values() {
			dot_cargo.insert_target(
				...
				target.generate_cargo_config(config, env).map_err(Error::DotCargoGenFailed)?,
			);
		}
	}
