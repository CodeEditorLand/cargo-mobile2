...
use crate::{
	android::{
		config::DEFAULT_VULKAN_VALIDATION,
		DEFAULT_ACTIVITY,
		DEFAULT_THEME_PARENT,
	},
	bicycle,
	dot_cargo,
	...
	fn report(&self) -> Report {
		match self {
			Self::RustupFailed(err) => {
				Report::error("Failed to `rustup` Android toolchains", err)
			},
			Self::MissingPack(err) => {
				Report::error("Failed to locate Android template pack", err)
			},
			...
			Self::DirectoryReadFailed { path, cause } => {
				Report::error(
					format!("Failed to read directory at {:?}", path),
					cause,
				)
			},
			Self::DirectoryRemoveFailed { path, cause } => {
				Report::error(
					format!(
						"Failed to remove directory directory at {:?}",
						path
					),
					cause,
				)
			...
			Self::AssetDirSymlinkFailed(err) => {
				Report::error(
					"Asset dir couldn't be symlinked into Android project",
					err,
				)
			},
...
pub fn gen(
	...
	bike.filter_and_process(
		...
		|map| {
			...
			let asset_packs =
				asset_packs.iter().map(|p| p.name.as_str()).collect::<Vec<_>>();
	...
	if !asset_packs.is_empty() {
		Report::action_request(
			"When running from Android Studio, you must first set your \
			 deployment option to \"APK from app bundle\".",
			"Android Studio will not be able to find your asset packs \
			 otherwise. The option can be found under \"Run > Edit \
	...
	for source in metadata.app_sources() {
		...
		fs::copy(&source_src, source_dest.join(source_file)).map_err(
			|cause| {
