...
impl<'a> Device<'a> {
	...
	fn install_apk(
		...
		let apk_path =
			apk::apks_paths(config, profile, flavor).into_iter().reduce(last_modified).unwrap();
	...
	pub fn run(
		...
		if build_app_bundle {
			...
			self.build_apks_from_aab(config, profile).map_err(RunError::ApksFromAabBuildFailed)?;
