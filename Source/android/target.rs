...
impl<'a> Target<'a> {
	...
	pub fn symlink_libs(
		...
		if needs_cxx_shared {
			...
			let cxx_shared =
				ndk.libcxx_shared_path(*self).map_err(SymlinkLibsError::LibcxxSharedPathFailed)?;
	...
	pub fn build(
		...
		self.symlink_libs(config, &env.ndk, profile).map_err(BuildError::SymlinkLibsFailed)
	}
}
