	...
	fn report(&self) -> Report {
		match self {
			...
			Self::Entry { dir, source } => {
				Report::error(format!("Failed to get entry in jniLibs directory {:?}", dir), source)
			}
...
pub fn path(config: &Config, target: Target<'_>) -> PathBuf {
	prefix_path(config.project_dir(), format!("app/src/main/jniLibs/{}", &target.abi))
}
...
impl JniLibs {
	...
	pub fn remove_broken_links(config: &Config) -> Result<(), RemoveBrokenLinksError> {
		for abi_dir in
			Target::all().values().map(|target| path(config, *target)).filter(|path| path.is_dir())
		{
