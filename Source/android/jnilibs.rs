	...
	fn report(&self) -> Report {
		match self {
			Self::ReadDir { dir, source } => {
				Report::error(
					format!(
						"Failed to list contents of jniLibs directory {:?}",
						dir
					),
					source,
				)
			...
			Self::Entry { dir, source } => {
				Report::error(
					format!(
						"Failed to get entry in jniLibs directory {:?}",
						dir
					),
					source,
				)
	...
	pub fn remove_broken_links(
			...
			.filter(|path| path.is_dir())
		{
			for entry in std::fs::read_dir(&abi_dir).map_err(|source| {
				RemoveBrokenLinksError::ReadDir { dir:abi_dir.clone(), source }
			})? {
				let entry = entry
					.map_err(|source| {
						RemoveBrokenLinksError::Entry {
							dir:abi_dir.clone(),
							source,
						}
					})?
				...
				if let Ok(path) = std::fs::read_link(&entry) {
					...
					if !path.exists() {
						log::info!(
							"deleting broken symlink {:?} (points to {:?}, \
							 which doesn't exist)",
							entry,
							path
						);
						std::fs::remove_file(entry).map_err(|source| {
							RemoveBrokenLinksError::Remove { path, source }
						})?;
					}
				}
			}
