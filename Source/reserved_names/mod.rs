...
pub fn in_keywords(s:impl AsRef<str>) -> bool { KEYWORDS.contains(&s.as_ref()) }

pub fn in_windows(s:impl AsRef<str>) -> bool { WINDOWS.contains(&s.as_ref()) }

pub fn in_artifacts(s:impl AsRef<str>) -> bool { ARTIFACTS.contains(&s.as_ref()) }
...
pub fn in_primitive_types(s:impl AsRef<str>) -> bool { PRIMITIVE_TYPES.contains(&s.as_ref()) }
