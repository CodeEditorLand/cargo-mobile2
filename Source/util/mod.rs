...
impl Display for PipeError {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			Self::TxCommandFailed(err) => {
				write!(f, "Failed to run sending command: {}", err)
			},
			Self::RxCommandFailed(err) => {
...
pub fn run_and_search<T>(
	...
	command
		...
		.map(|output| {
			re.captures(&output)
				.ok_or_else(|| {
					RunAndSearchError::SearchFailed {
...
pub fn get_string_for_group(
	...
	Ok(caps
		.name(group)
		.ok_or_else(|| {
			CaptureGroupError::InvalidCaptureGroup {
