...
pub fn rustup_add(triple: &str) -> Result<ExitStatus, std::io::Error> {
	duct::cmd("rustup", ["target", "add", triple]).dup_stdio().run().map(|o| o.status)
}
...
pub enum VersionTripleError {
	...
	PatchInvalid { version: String, source: std::num::ParseIntError },
	#[error(
...
impl FromStr for VersionTriple {
	...
	fn from_str(v: &str) -> Result<Self, Self::Err> {
		match v.split('.').count() {
			1 => Ok(VersionTriple {
				major: v.parse().map_err(|source| VersionTripleError::MajorInvalid {
					version: v.to_owned(),
					source,
				})?,
				minor: 0,
...
impl FromStr for VersionDouble {
	...
	fn from_str(v: &str) -> Result<Self, Self::Err> {
		match v.split('.').count() {
			1 => Ok(VersionDouble {
				major: v.parse().map_err(|source| VersionDoubleError::MajorInvalid {
					version: v.to_owned(),
					source,
				})?,
				minor: 0,
...
pub enum RustVersionError {
	...
	YearInvalid { date: String, source: std::num::ParseIntError },
	#[error("Failed to parse rustc release month from {date:?}: {source}")]
	MonthInvalid { date: String, source: std::num::ParseIntError },
	#[error("Failed to parse rustc release day from {date:?}: {source}")]
	DayInvalid { date: String, source: std::num::ParseIntError },
}
...
pub fn command_present(name: &str) -> Result<bool, std::io::Error> {
	command_path(name).map(|_path| true).or_else(|_err| Ok(false))
}
	...
	E: StdError,
{
	...
	CurrentDirSetFailed { path: PathBuf, source: std::io::Error },
	#[error(transparent)]
