	...
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			...
			Self::FetchFailed(err) => {
				write!(f, "Failed to fetch repo: {}", err)
			},
			Self::RevParseLocalFailed(err) => {
				write!(f, "Failed to get checkout revision: {}", err)
			},
			...
			Self::ResetFailed(err) => {
				write!(f, "Failed to reset repo: {}", err)
			},
			Self::CleanFailed(err) => {
				write!(f, "Failed to clean repo: {}", err)
			},
		}
	}
}
...
impl Status {
	pub fn stale(self) -> bool { matches!(self, Self::Stale) }

	pub fn fresh(self) -> bool { matches!(self, Self::Fresh) }
}
...
impl Repo {
	...
	pub fn git(&self) -> Git<'_> { Git::new(self.path()) }
