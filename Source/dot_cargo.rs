...
pub enum LoadError {
	...
	DeserializeFailed { path: PathBuf, cause: toml::de::Error },
}
...
impl DotCargo {
	fn create_dir_and_get_path(app: &App) -> Result<PathBuf, (PathBuf, io::Error)> {
		...
		fs::create_dir_all(&dir).map(|()| dir.join("config.toml")).map_err(|cause| (dir, cause))
	}
	...
	pub fn load(app: &App) -> Result<Self, LoadError> {
		...
		let old_style =
			path.parent().expect("developer error: cargo config path had no parent").join("config");
