...
impl Display for Invalid {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			...
			Self::NotAlphanumericHyphenOrUnderscore { app_name, naughty_chars, .. } => {
				write!(
					f,
					"\"{}\" contains {}, but only lowercase letters, numbers, hyphens, and \
...
fn normalize_case(s:&str) -> String {
	if s.contains('_') { s.to_snek_case() } else { s.to_kebab_case() }
}
...
pub fn validate<T:Deref<Target = str>>(app_name:T) -> Result<T, Invalid> {
	...
	if let Err(err) = result.as_mut() {
		...
		match err {
			...
			Invalid::NotAlphanumericHyphenOrUnderscore { app_name, suggested, .. } => {
				*suggested =
