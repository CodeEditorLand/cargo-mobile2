...
pub fn file_name() -> String { format!("{}.toml", crate::NAME) }
...
impl Reportable for LoadOrGenError {
	fn report(&self) -> Report { Report::error("Config error", self) }
}
...
impl Origin {
	pub fn freshly_minted(self) -> bool { matches!(self, Self::FreshlyMinted) }
}
...
impl Config {
	...
	pub fn path(&self) -> PathBuf { self.app().root_dir().join(file_name()) }

	pub fn app(&self) -> &App { &self.app }

	#[cfg(target_os = "macos")]
	pub fn apple(&self) -> &apple::config::Config { &self.apple }
