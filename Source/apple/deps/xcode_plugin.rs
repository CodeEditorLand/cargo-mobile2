...
impl Display for Error {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			...
			Self::XcodeSelectFailed(err) => {
				write!(f, "Failed to get path to Xcode.app: {}", err)
			},
			Self::StatusFailed(err) => write!(f, "{}", err),
			Self::UpdateFailed(err) => write!(f, "{}", err),
			Self::UuidLookupFailed(err) => {
				write!(f, "Failed to lookup Xcode UUID: {}", err)
			},
			...
			Self::MetaCopyFailed(err) => {
				write!(f, "Failed to copy language metadata: {}", err)
			},
		}
	}
}
