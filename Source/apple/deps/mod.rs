...
pub enum Error {
	...
	PresenceCheckFailed { package:&'static str, source:std::io::Error },
	#[error("Failed to install `{package}`: {source}")]
	InstallFailed { package:&'static str, source:std::io::Error },
...
fn installed_with_brew(package:&str) -> bool {
	duct::cmd("brew", ["list", package]).dup_stdio().run().is_ok()
}
...
impl PackageSpec {
	pub const fn brew(pkg_name:&'static str) -> Self {
		Self { pkg_name, bin_name:pkg_name, package_source:PackageSource::Brew }
	}
