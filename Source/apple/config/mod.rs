...
impl Platform {
	...
	pub fn cargo_args(&self) -> Option<&[String]> { self.cargo_args.as_deref() }
...
impl Metadata {
	pub const fn supported(&self) -> bool { self.supported }

	pub fn ios(&self) -> &Platform { &self.ios }

	pub fn macos(&self) -> &Platform { &self.macos }
}

#[derive(Debug)]
pub enum ProjectDirInvalid {
	NormalizationFailed { project_dir:String, cause:util::NormalizationError },
	OutsideOfAppRoot { project_dir:String, root_dir:PathBuf },
}
...
impl Error {
	pub fn report(&self, msg:&str) -> Report { Report::error(msg, self) }
}
...
impl Config {
	...
	pub fn archive_dir(&self) -> PathBuf { self.project_dir().join("build") }

	pub fn export_dir(&self) -> PathBuf { self.project_dir().join("build") }
