	...
	util::RustVersion::check().map(|version| {
		if !version.valid() {
			Report::action_request(
				format!(
					"iOS linking is broken on Rust versions later than 1.45.2 (d3fb005a3 \
					 2020-07-31) and earlier than 1.49.0-nightly (ffa2e7ae8 2020-10-24), but \
					 you're on {}!",
					version
				),
				"This is fixed in Rust 1.49.0 and later:\n`rustup update stable && rustup default \
				 stable`",
			)
			.print(wrapper);
		}
