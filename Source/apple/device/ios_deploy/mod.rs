...
enum Event {
	...
	#[allow(dead_code)]
	BundleCopy { percent:u32, overall_percent:u32, path:PathBuf },
	#[serde(rename_all = "PascalCase")]
	#[allow(dead_code)]
	BundleInstall { percent:u32, overall_percent:u32, status:String },
	#[serde(rename_all = "PascalCase")]
...
impl Event {
	...
	fn device_info(&self) -> Option<&DeviceInfo> {
		if let Self::DeviceDetected { device } = self { Some(device) } else { None }
	}
}
