...
impl Reportable for DeviceListError {
	fn report(&self) -> Report { Report::error("Failed to detect connected iOS devices", self) }
}
...
fn parse_device_list<'a>(
	...
	Event::parse_list(&stdout)
		...
		.map(|DeviceInfo { device_identifier, device_name, model_arch, model_name }| {
...
pub fn device_list<'a>(env:&Env) -> Result<BTreeSet<Device<'a>>, DeviceListError> {
	let result = duct::cmd("ios-deploy", ["--detect", "--timeout", "1", "--json", "--no-wifi"])
		.stdout_capture()
		.stderr_capture()
