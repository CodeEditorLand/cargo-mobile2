...
fn parse_quoted_text(
	...
	for &c in text.as_bytes() {
		if escaping {
			...
			// escaping mode if yes (dropping c). Otherwise just pass the c
			// char.
			if c == b'\\' {
				escaping = true;
			} else {
...
pub fn parse_command(
	...
	// regex replacements on the arguments individually. The spec itself says
	...
	for &c in command.as_bytes() {
		...
		// Otherwise, we have to pay special attention to backslash
			...
			// "read" the backslash, and instead escape the next char.
			escape_char = true;
