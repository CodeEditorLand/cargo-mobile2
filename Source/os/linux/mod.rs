...
impl Application {
	...
	pub fn open_file(&self, path:impl AsRef<Path>) -> Result<(), OpenFileError> {
		...
		if !command_parts.is_empty() {
			...
			// single element, &command_parts[1..] should be an empty slice
			// (&[]) and duct does not add any argument on that case
...
pub fn code_command() -> duct::Expression { duct::cmd!("code") }

pub fn replace_path_separator(path:OsString) -> OsString { path }
