...
pub fn check() -> Result<Info, Error> {
	...
	let condition_mask = unsafe {
		VerSetConditionMask(condition_mask, VER_PRODUCT_TYPE, VER_EQUAL as u8)
	};
	...
	for &(name, major, minor, service_pack, product_type, build_number) in
		VERSION_LIST
	{
		...
		if unsafe {
			VerifyVersionInfoW(&mut osvi as *mut _, type_mask, condition_mask)
		}
		.is_ok()
		{
			return Ok(Info {
