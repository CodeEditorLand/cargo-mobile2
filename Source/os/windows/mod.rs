...
impl From<core::Error> for DetectEditorError {
	fn from(err:core::Error) -> Self { Self::IOError(err.into()) }
}
...
impl Drop for NativeArgv {
	fn drop(&mut self) { let _ = unsafe { LocalFree(HLOCAL(self.argv as _)) }; }
}
...
pub fn code_command() -> duct::Expression { duct::cmd!("code.cmd") }
