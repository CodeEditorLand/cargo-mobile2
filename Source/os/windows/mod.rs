...
impl Application {
	...
	pub fn open_file(&self, path: impl AsRef<Path>) -> Result<(), OpenFileError> {
		let args = self.argv[1..]
			...
			.collect::<Vec<_>>();
		duct::cmd(&self.argv[0], args).run_and_detach().map_err(OpenFileError::LaunchFailed)?;
		Ok(())
	}
...
fn open_file_with_android_studio(path: impl AsRef<OsStr>, env: &Env) -> Result<(), OpenFileError> {
	...
	duct::cmd(
		application_path,
		[dunce::canonicalize(Path::new(path.as_ref())).expect("Failed to canonicalize file path")],
	)
