...
impl Pack {
	...
	pub fn lookup_platform(name: &str) -> Result<Self, LookupError> {
		platform_pack_dir().map_err(LookupError::NoHomeDir).and_then(|dir| Self::lookup(dir, name))
	}

	pub fn lookup_app(name: &str) -> Result<Self, LookupError> {
		app_pack_dir().map_err(LookupError::NoHomeDir).and_then(|dir| Self::lookup(dir, name))
	}
