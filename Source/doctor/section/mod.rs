...
impl<T:ToString, E:ToString> From<Result<T, E>> for Item {
	fn from(result:Result<T, E>) -> Item { Self::from_result(result) }
}
...
impl Item {
	...
	#[cfg(target_os = "macos")]
	fn warning(msg:impl ToString) -> Self { Self::new(Label::Warning, msg) }

	fn failure(msg:impl ToString) -> Self { Self::new(Label::Error, msg) }
	...
	fn is_warning(&self) -> bool { matches!(self.label, Label::Warning) }
...
impl Section {
	fn new(title:impl ToString) -> Self {
		Self { title:title.to_string(), items:Default::default() }
	}
	...
	fn has_error(&self) -> bool { self.items.iter().any(Item::is_failure) }

	fn has_warning(&self) -> bool { self.items.iter().any(Item::is_warning) }
