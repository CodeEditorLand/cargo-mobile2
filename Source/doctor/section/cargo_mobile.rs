...
fn check_rust() -> Result<String, String> {
	util::RustVersion::check().map_err(|err| err.to_string()).and_then(|version| {
		version.valid().then(|| format!("rustc v{}", version)).ok_or_else(|| {
			format!("iOS linking is broken on rustc v{}; please update to 1.49.0 or later", version)
		})
	})
}
		...
		Ok(install_dir) => {
			section
				...
				.with_item(if install_dir.exists() {
					Ok(format!("Installed at {:?}", util::contract_home(&install_dir)?,))
				} else {
					Err(format!(
