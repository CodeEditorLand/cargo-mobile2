...
fn check_rust() -> Result<String, String> {
	util::RustVersion::check().map_err(|err| err.to_string()).and_then(
		|version| {
			version.valid().then(|| format!("rustc v{}", version)).ok_or_else(
				|| {
					format!(
						"iOS linking is broken on rustc v{}; please update to \
						 1.49.0 or later",
						version
					)
				},
		...
		Ok(install_dir) => {
			section
				.with_item(util::installed_commit_msg().map(|msg| {
					msg.map(util::format_commit_msg).unwrap_or_else(|| {
						"Installed commit message isn't present".to_string()
					})
				}))
				.with_item(if install_dir.exists() {
					...
					Err(format!(
						"The cargo-mobile2 installation directory is missing! \
						 Checked at {:?}",
						install_dir,
					))
				})
